...
const ADAPTER_REQUIREMENTS: Record<AdapterType, Record<AdapterDirection, { required: string[]; optional: Record<string, string>; example?: Record<string, any>; notes?: string }>> = {
    ...
    'AMQP': {
        ...
        'Sender': {
            ...
            notes: 'host, port, and credentialName are REQUIRED and must never be empty -- SAP rejects a missing/blank value for any of them on import ("Attribute \'Host\' is mandatory", "Attribute \'Credential Name\' is mandatory", "Enter a value between 1 and 65535" for port). port must be an integer between 1 and 65535. Use the EXACT property names host/port/credentialName -- do NOT invent alternatives such as hostname, server, credential, credentialId, or credentialAlias. If the real Event Mesh host/port/credential is not known at generation time (the common case -- this is tenant-specific infrastructure data, never invent a real-looking value), supply a SAP-style externalized-parameter placeholder string instead of a literal value, e.g. host: "{{EMHOST}}", port: "{{EMPORT}}", credentialName: "{{EMUser}}" -- this is the exact convention the reference SAP export itself uses. The compiler automatically registers each "{{...}}" placeholder as a real SAP externalized parameter (parameters.prop/parameters.propdef entries matching the reference\'s own structure) AND fills in a non-empty, clearly-fake default value for it (e.g. "your-event-mesh-host.example.com" for host, 443 for port, "REPLACE_WITH_CREDENTIAL_NAME" for credentialName) -- confirmed by live SAP import that externalizing a field does NOT exempt it from needing a real (even if placeholder) value; SAP resolves and validates the placeholder\'s underlying value, it does not skip validation just because a <reference> exists. The user replaces this default with the real value after import via Configure > Externalized Parameters. A literal, valid port number (1-65535) is also accepted directly when known.'
        }
    } as any
};
